...
Die persönliche Bereichsnavigation gehört auf App-Ebene zu `src/App.tsx` und bleibt nach erfolgreichem Login, Datenfreigabe und gültiger Rolle dauerhaft sichtbar. Sie wechselt zwischen `Entdecken`, `Favoriten`, dem rollenabhängigen Bereich `Meine Kunstwerke` bzw. `Meine Anfragen` und `Profil`. Auf kleinen Bildschirmen liegt sie fest am unteren Bildschirmrand und berücksichtigt den iOS-Safe-Area-Abstand. `src/gallery/Gallery.tsx` rendert nur noch den aktuell gewählten Bereich und erhält dessen Zustand von `App.tsx`. Dadurch ist die Navigation unabhängig von Scrollposition und Galerie-Inhalt und die vorhandenen Google-Daten müssen trotzdem nicht mehrfach geladen werden.
