...
Anmeldung und API-Zugriff sind getrennt: `src/auth/googleAccess.ts` verwendet `google.accounts.oauth2.initTokenClient`, um kurzlebige Access Tokens für Google Sheets und Google Drive anzufordern. Nach einem erfolgreichen Google-ID-Login stößt `src/App.tsx` diesen zweiten Google-Schritt unmittelbar im Login-Callback an, sodass die Datenfreigabe ohne zusätzliches Scrollen oder vorherigen Klick erscheint. Der sichtbare Freigabe-Button bleibt als Fallback erhalten, falls das zweite Google-Fenster geschlossen, abgebrochen oder vom Browser verhindert wird. Vor dem Zugriff auf die Fachdaten wird über Google UserInfo geprüft, dass der für die APIs autorisierte Account dieselbe E-Mail-Adresse wie der aktive Login besitzt.
