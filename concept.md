...
Nach einer erfolgreichen Google-Anmeldung fordert RentArt direkt im Anschluss automatisch den zusätzlichen Google-API-Zugriff für Sheets und Drive an. Login und Datenfreigabe bleiben technisch zwei getrennte Google-Schritte, erscheinen für den Nutzer aber unmittelbar nacheinander. Dabei muss derselbe Google-Account verwendet werden. Falls das zweite Google-Fenster vom Browser verhindert, geschlossen oder abgebrochen wird, bleibt im Galerie-Bereich der Button `Google-Daten freigeben` als Fallback sichtbar.
