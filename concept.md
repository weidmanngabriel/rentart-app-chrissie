...
Nach der Anmeldung wird für den Zugriff auf das Google-Backend zusätzlich ein kurzlebiger Google-API-Zugriff angefordert. Dabei muss derselbe Google-Account verwendet werden. Ein noch gültiger Zugriff wird zusammen mit seiner Ablaufzeit lokal auf dem Gerät gespeichert, sodass das Schließen und erneute Öffnen der installierten PWA die Datenfreigabe nicht sofort verliert. Ist das Token tatsächlich abgelaufen oder wurde es von Google ungültig gemacht, wird es entfernt und der Nutzer muss den Datenzugriff erneut per Klick aktivieren. Die bereits bei Google erteilte Zustimmung zu den Scopes bleibt davon getrennt bestehen.
