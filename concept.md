...
Nach der Anmeldung wird für den Zugriff auf das Google-Backend zusätzlich ein kurzlebiger Google-API-Zugriff angefordert. Dabei muss derselbe Google-Account verwendet werden. Dieser API-Zugriff wird bewusst nicht dauerhaft gespeichert. Nach einem vollständigen Browser- oder PWA-Neustart kann deshalb erneut ein kurzlebiger Google-Datenzugriff nötig sein, ohne dass der Nutzer sich erneut bei RentArt anmelden oder sein Konto neu auswählen muss. Abgelaufene Tokens müssen für das jeweilige Konto neu freigegeben werden.
